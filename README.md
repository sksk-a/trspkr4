...
3. Файл `.env.example` и `.gitignore`.
4. Папку `alembic/versions` с двумя миграциями.
5. Запуск `alembic upgrade head`.
6. Swagger UI по адресу `/docs`.
7. Примеры запросов через Swagger.
8. Запуск `pytest`, где все тесты должны быть `passed`.
